...
void print_menu(int highlight, const char *choices[], int n_choices) {
    ...
    // Exibe as opções do menu com espaçamento adequado
    for (int i = 0; i < n_choices; ++i) {
        gotoxy(8, 6 + i);  // Posiciona o cursor para cada opção
        if (highlight == i) {
        	set_color(15, 0);
            // Destaca a opção selecionada com setas
            printf("|  > %-21s|\n", choices[i]);
        } else {
            printf("|    %-21s|\n", choices[i]);
        }
    ...
    set_color(7, 0);  // Cor padrão para o texto da frase
    printf("USE AS SETAS PARA NAVEGAR E ENTER PARA SELECIONAR\n");
}



// Função para criptografar a senha (simples deslocamento de caracteres)
...
void cadastrarUsuario(const char *nome, const char *senha) {
    ...
    char senha_cripto[50];  // Variável para armazenar a senha criptografada
...
void listarUsuarios() {
    ...
    while (fgets(linha, sizeof(linha), arquivo)) { // Lê a linha inteira
...
void excluirUsuario(const char *nome) {
    ...
    if (encontrado) {
        ...
        // Chama a função novamente com o novo nome
...
int main() {
    ...
    if (choice == 0) {                   
    	...
    	reset_color();
    	fgets(nome[0], sizeof(nome[0]), stdin);  // Usa fgets para capturar o nome com espaços
